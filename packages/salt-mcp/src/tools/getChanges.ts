...
function resolveComponentTarget(
  ...
  for (const component of componentsByNormalizedName.get(normalizedTargetName) ??
    []) {
  ...
  for (const component of componentsByNormalizedAlias.get(normalizedTargetName) ??
    []) {
    if (packageName && component.package.name !== packageName) {
      continue;
    }
...
export function getChanges(
  ...
  if (input.target_type) {
    changes = changes.filter((change) => change.target_type === input.target_type);
  }
  if (input.package) {

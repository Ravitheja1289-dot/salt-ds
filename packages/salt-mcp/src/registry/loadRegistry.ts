...
async function loadPageSearchIndex(
  ...
  try {
    const artifact = await readJsonFile<PageSearchIndexArtifact>(
      pageSearchIndexPath,
    );
    ...
    if (typeof artifact.version !== "string" || artifact.version.length === 0) {
      throw new Error("page-search-index.json is missing a valid version field.");
    }
    if (
...
async function loadRegistryArrayArtifact<
  Key extends RegistryArrayArtifactKey,
>(
  registryDir: string,
...
export async function loadRegistry(
  ...
  const arrayArtifacts = Object.fromEntries(arrayArtifactEntries) as {
    [Key in RegistryArrayArtifactKey]: LoadedArtifact<SaltRegistry[Key][number]>;
  };

...
Generate registry artifacts from repo sources:

```sh
node dist/salt-ds-mcp/bin/salt-mcp.js build-registry \
...
```sh
node dist/salt-ds-mcp/bin/salt-mcp.js serve
```

Override the public docs base URL used when expanding `/salt/...` routes in top-level `sources`:

```sh
node dist/salt-ds-mcp/bin/salt-mcp.js serve \
  --site-base-url https://www.saltdesignsystem.com
```
